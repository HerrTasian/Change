...
function link(path, label, className = '') {
  ...
  return `<a href="${path}" class="${className} ${active ? 'active' : ''}" data-link>${escapeHtml(label)}</a>`;
}
  ...
  return `
    ...
    <header class="site-header">
      <nav class="nav-container" aria-label="${escapeHtml(text.ui.primaryNavigation)}">
        <a class="brand" href="/" data-link aria-label="${escapeHtml(text.ui.homeLabel)}">
    ...
    <footer class="footer"><div><strong>${escapeHtml(text.brand.name)}</strong><p>${escapeHtml(text.footer.note)}</p></div><a class="footer-link" href="/concept" data-link>${escapeHtml(text.footer.cta)} →</a></footer>`;
}
...
function overviewPage() {
  ...
  return `
    ...
    <section class="page-section tinted-section">${sectionHeader(text.overview.navigationTeaser, 'center')}<div class="route-grid">${text.navigation.filter((item) => item.path !== '/').map((item) => `<a class="route-card" href="${item.path}" data-link><span>${escapeHtml(item.label)}</span><span aria-hidden="true">→</span></a>`).join('')}</div></section>`;
}
